...
/**
 * Sharable implementation to "bootstrap" client SMPP sessions (create & bind).
 */
public class SharedSmppClient implements SmppClient {
    private static final Logger logger = LoggerFactory.getLogger(SharedSmppClient.class);
     ...
     */
    public SharedSmppClient(ExecutorService executor) {
     ...
     */
    public SharedSmppClient(ExecutorService executor, int expectedSessions) {
        this(executor, expectedSessions, null);
    }
     ...
     *                         be disabled.
     */
    public SharedSmppClient(ExecutorService executor, int expectedSessions, ScheduledExecutorService monitorExecutor) {
        this.channels = new DefaultChannelGroup();
        this.executor = executor;
        this.channelFactory = new NioClientSocketChannelFactory(this.executor, this.executor, expectedSessions);
    ...
    protected Channel createConnectedChannel(String host, int port, long connectTimeoutMillis) throws SmppTimeoutException, SmppChannelException, InterruptedException {
        ...
        clientBootstrap.getPipeline().addLast(SmppChannelConstants.PIPELINE_CLIENT_CONNECTOR_NAME, this.clientConnector);
        // set the timeout
        clientBootstrap.setOption("connectTimeoutMillis", connectTimeoutMillis);

        // attempt to connect to the remote system
        ChannelFuture connectFuture = clientBootstrap.connect(socketAddr);
